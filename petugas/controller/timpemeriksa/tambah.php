...
<div class="main-content">
							...
							<form method="post" action="simpan.php" enctype="multipart/form-data">
										...
										<div class="widget-box widget-color-blue2">
											<div class="widget-header">
												...
												<h4 class="widget-title lighter smaller">Form Untuk Menambah  Tim Pemeriksa</h4>
											</div>
    ...
    $jsArray .= "prdName['" . $r['notlhp'] . "'] = {tgl_tlhp:'" . addslashes($r['tgl_tlhp']) . "',nolaporan:'".addslashes($r['nolaporan'])."',nama:'".addslashes($r['nama'])."',jumlah_tim:'".addslashes($r['jumlah_tim'])."'};\n";    
														...
														<div class="space-4"></div>
														<div class="form-group">
															<label for="form-field-select-3">Jumlah Tima Yang Menghadapi Kasus Ini</label>
									...
									<div class="col-sm-3">
										...
										<div class="widget-box widget-color-blue2">
											...
											<form method="POST" action="#">
														<div class="form-group">
														<label for="form-field-select-3">TambahKan Tim</label>
														
														<div>
															 <a href="#"> <button type="button" class="btn btn-info" id="tambahtim"><span class="glyphicon glyphicon-user">Tambah tim </span></button></a>
															 </div>
														</div>
														<p id="tim">
...
foreach ((array)@$_POST['kdpenyidik'] as $row => $act) {
	//session_start();
	$kdpenyidik=trim($_POST['kdpenyidik'][$row]);
	$jabatan=trim($_POST['jabatan'][$row]);
	$temp['kde']=$kdpenyidik;
	$temp['jbt']=$jabatan;
...
?>
															...
															<tbody>
																	...
																	foreach ((array)$_SESSION['temp_var'] as $value) {?>
																		<tr>
																		<td><?php echo $value['kde'] ?></td>
																		<td><?php echo $value['jbt'] ?></td>
																		</tr>
																<?php	}
																	?>
															</tbody>
	   ...
	   var count=0;
        $(function(){
            $('#tambahtim').click(function(){
                ...
                $('#tim').append(
                		'<div class="form-group">'
                       +'<label for="nama">Kode Penyidik #' + count + '</label>' 
				+ '<select id="kdpenyidik_' + count + '" name="kdpenyidik[]'+'"  class="chosen-select form-control">'
				+ '<optiongroup>'
				+ '<option value="0">Pilih TIm Pemeriksa</option>'
				<?php
					$tampil=mysqli_query($koneksi,"SELECT * FROM penyidik ORDER BY kdpenyidik");
					while($r=mysqli_fetch_array($tampil)){ ?>
				+ '	<option value="<?php echo $r['kdpenyidik'] ?>" ><?php echo $r['nama'] ?></option>'
				<?php } ?>
				+ '</optiongroup>' 
				+ '</select>'
				+'</div>'
				...
				+'<div class="form-group">'
                       +'<label for="nama">Jabatan #' + count + '</label>' 
				+ '<input id="jabatan_' + count + '" name="jabatan[]' + '" type="text" class="form-control" />' 
				+'</div>'
